...
export type IHendricksOptions = { plugins?: Plugin[], token: string, guildId: string, clientId: string };
...
export default class Hendricks {
	...
	constructor(options: IHendricksOptions) {
		...
		this.client.on('interactionCreate', async (...args: any[]) => {
			...
			if (interaction.isChatInputCommand()) {
				const command = this.commandRegistry.commands.find((command) => {
	...
	deployCommands({ target }: { target: 'GUILD' | 'GLOBAL' }) {
		...
		const applicationCommands = this.commandRegistry.commands.map((command) =>
		...
		rest.put(route, {
			...
			.then((data) =>
				this.logger.info(
					`Successfully registered ${(data as any[]).length} ${target === 'GUILD' ? 'guild' : 'application'
					} commands.`
				)
			)
			.catch(console.error);
	...
	start() {
		...
		this.events.on('plugins:init', () => {
			this.logger.info(`🧩 ${this.pluginManager.plugins.length} Plugins initialized`);

			this.events.emit('plugins:start');
		});

		this.events.on('register:commands', () => {
			this.logger.info(`💬 ${this.commandRegistry.commands.length} Commands registered`);
		});
	}
